            ...
            while (true) {
                if (clipQueue.Count > 0 || interrupt) {
                    ...
                    if (clipQueue.Count == 0 && !interrupt) {
                        ...
                        {
                            if (Soul.instance.isReached)
                            {

                                Soul.instance.memoryPanel.SetActive(true);

                                foreach (GameObject memory in Soul.instance.memoryList)
                                {
                                    if (memory.GetComponent<Orb>().getIsCollected())
                                        Soul.instance.listViewManager.AddRow(SingleRowData(memory.GetComponent<Orb>().getMemoryText()));
                                    else
                                    {
                                        Soul.instance.listViewManager.AddRow(SingleRowData("Not Collected"));
                                    }
